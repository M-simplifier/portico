...
import Portico.Site (AssetTarget(..), Block(..), CalloutTone(..), LinkCard, LinkTarget(..), NavItem, Page, PageKind(..), Section, Site, pagePath)
...
renderDocument :: RouteContext -> StyleStrategy -> Theme -> Site -> Page -> String
renderDocument routeContext styleStrategy currentTheme currentSite currentPage =
  let
  ...
  joinWith ""
    ...
    , renderSiteHeader routeContext currentSite
    , "<main id=\"main-content\" class=\"page-main\">"
    , renderPageIntro currentPage
    , foldMap (renderSection routeContext currentPage) currentPage.sections
    , "</main>"
...
renderPageAtMount mountPath currentTheme currentSite currentPage =
  ...
  in
    ...
    , html: renderDocument routeContext InlineStyles currentTheme currentSite currentPage
    }
...
renderSiteHeader :: RouteContext -> Site -> String
renderSiteHeader routeContext currentSite =
  joinWith ""
...
renderPageIntro currentPage =
  if hasLeadHero currentPage then
    ...
    joinWith ""
      ...
      , "<p class=\"page-kind\">"
      , escapeHtml (pageKindLabel currentPage.kind)
      , "</p>"
      , "<h1>"
...
renderSection :: RouteContext -> Page -> Section -> String
renderSection routeContext currentPage currentSection =
  joinWith ""
    [ "<section"
    ...
    , foldMap (renderBlock routeContext currentPage) currentSection.blocks
    , "</div>"
    , "</section>"
    ]
...
renderBlock :: RouteContext -> Page -> Block -> String
renderBlock routeContext currentPage = case _ of
  HeroBlock currentHero ->
    joinWith ""
      [ "<article class=\"hero-block block-card\">"
      , renderHeroEyebrow currentPage currentHero.eyebrow
      , "<h1>"
      ...
      , "</p>"
      , renderHeroActions routeContext currentHero.actions
      , "</article>"
      ]
  ProseBlock body ->
  ...
  CalloutBlock currentCallout ->
    joinWith ""
      ...
      , escapeHtml (calloutToneLabel currentCallout.tone)
      , "</p>"
      , "<h3>"
  ...
  LinkGridBlock cards ->
    joinWith ""
      [ "<div class=\"link-grid\">"
      , foldMap (renderLinkCard routeContext) cards
      , "</div>"
      ]

renderHeroEyebrow :: Page -> Maybe String -> String
renderHeroEyebrow currentPage eyebrow =
  case eyebrow of
    Just value -> "<p class=\"hero-eyebrow\">" <> escapeHtml value <> "</p>"
    Nothing -> "<p class=\"hero-eyebrow\">" <> escapeHtml (pageKindLabel currentPage.kind) <> "</p>"

renderHeroActions :: RouteContext -> Array LinkCard -> String
renderHeroActions routeContext actions =
  case actions of
    [] -> ""
    _ ->
      joinWith ""
        [ "<div class=\"hero-actions\">"
        , foldMap (renderLinkCard routeContext) actions
        , "</div>"
        ]
...
renderLinkCard :: RouteContext -> LinkCard -> String
renderLinkCard routeContext currentCard =
  joinWith ""
    ...
    , "<span class=\"link-card-meta\">Open</span>"
    , "</a>"
    ]
...
hasLeadHero currentPage =
  case head currentPage.sections of
    Just currentSection ->
      case head currentSection.blocks of
        Just (HeroBlock _) -> true
        _ -> false
    Nothing ->
      false

pageKindLabel :: PageKind -> String
pageKindLabel = case _ of
  Landing -> "Landing"
...
calloutToneLabel :: CalloutTone -> String
calloutToneLabel = case _ of
  Quiet -> "Note"
  Accent -> "Highlight"
  Strong -> "Important"
